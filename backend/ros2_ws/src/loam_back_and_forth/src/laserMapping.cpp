...
// Kd-trees for corner and surface
...
// Pose variables (x, y, z, roll, pitch, yaw)
...
void laserOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr laserOdometry)
{
  ...
  geometry::msgs::Quaternion geoQuat = laserOdometry->pose.pose.orientation;
...
int main(int argc, char** argv)
{
  ...
  //                                    ("/laser_cloud_last_2", 2, laserOdometryHandler);
  auto subLaserCloudLast2 = node->create_subscription<sensor_msgs::msg::PointCloud2>(
    "/laser_cloud_last_2", 2, laserCloudLastHandler);

  //ros::Subscriber subLaserOdometry = nh.subscribe<nav_msgs::Odometry>
  //                                    ("/cam_to_init", 5, laserCloudLastHandler);
  ...
  auto pubLaserCloudSurround = node->create_publisher<sensor_msgs::msg::PointCloud2>(
  ...
  //ros::Publisher pubOdomBefMapped = nh.advertise<nav::Odometry> ("/bef_mapped_to_init_2", 5);
  auto pubOdomBefMapped = node->create_publisher<nav_msgs::msg::Odometry>(
    "/bef_mapped_to_init_2", 5);
  nav_msgs::msg::Odometry odomBefMapped;
  odomBefMapped.header.frame_id = "camera_init_2";
  odomBefMapped.child_frame_id = "bef_mapped";

  //ros::Publisher pubOdomAftMapped = nh.advertise<nav::Odometry> ("/aft_mapped_to_init_2", 5);
  auto pubOdomAftMapped = node->create_publisher<nav_msgs::msg::Odometry>(
    "/aft_mapped_to_init_2", 5);
              ...
              for (int ii = -1; ii <= 1; ii += 2) {
                for (int jj = -1; jj <= 1; jj += 2) {
                  for (int kk = -1; kk <= 1; kk += 2) {
                    ...
                    float check = 100.0
                                + std::pow(transformTobeMapped[3] - cornerX, 2)
                                + std::pow(transformTobeMapped[4] - cornerY, 2)
                                + std::pow(transformTobeMapped[5] - cornerZ, 2)
                                - std::pow(pointOnZAxis.x - cornerX, 2)
                                - std::pow(pointOnZAxis.y - cornerY, 2)
                                - std::pow(pointOnZAxis.z - cornerZ, 2);
      ...
      *laserCloudLast = *laserCloudCorner2 + *laserCloudSurf2;
      ...
      if (laserCloudSurfFromMap->points.size() > 500) {
        ...
        for (int iterCount = 0; iterCount < 10; iterCount++) {
          ...
          for (int i = 0; i < laserCloudLastNum; i++) {
            if (std::fabs(laserCloudLast->points[i].x > 0.3) || 
                std::fabs(laserCloudLast->points[i].y > 0.3) ||
              ...
              if (std::fabs(pointOri.v) < 0.05 || std::fabs(pointOri.v + 1) < 0.05) {
                ...
                if (pointSearchSqDis[4] < 1.0) {
                  ...
                  for (int j = 0; j < 5; j++) {
                    float dist = std::fabs(
                      pa * laserCloudSurfFromMap->points[pointSearchInd[j]].x +
                      pb * laserCloudSurfFromMap->points[pointSearchInd[j]].y +
                      pc * laserCloudSurfFromMap->points[pointSearchInd[j]].z + pd);
                    if (dist > 0.05) {
                      planeValid = false;
                      break;
                    }
                ...
                if (planeValid) {
                  ...
                  float s = 1;
                  if (iterCount >= 6) {
                    s = 1 - 8 * std::fabs(pd2) / sqrt(sqrt(pointSel.x * pointSel.x
          ...
          if (std::fabs(matX.at<float>(0, 0)) < 0.5 &&
              std::fabs(matX.at<float>(1, 0)) < 0.5 &&
              std::fabs(matX.at<float>(2, 0)) < 0.5 &&
              std::fabs(matX.at<float>(3, 0)) < 1 &&
              std::fabs(matX.at<float>(4, 0)) < 1 &&
              std::fabs(matX.at<float>(5, 0)) < 1) {
      ...
      for (int i = 0; i < laserCloudValidNum; i++) {
        ...
        for (int j = 0; j < laserCloudCubeNum; j++) {
          if (std::fabs(laserCloudCubePointer->points[j].v - 2) < 0.005 || 
              std::fabs(laserCloudCubePointer->points[j].v - 1) < 0.005) {
      ...
      odomBefMapped.pose.pose.orientation.x = -geoQuatB.y();
      odomBefMapped.pose.pose.orientation.y = -geoQuatB.z();
      odomBefMapped.pose.pose.orientation.z = geoQuatB.x();
      odomBefMapped.pose.pose.orientation.w = geoQuatB.w();
      ...
      //geoQuat = tf::createQuaternionMsgFromRollPitchYaw(transformAftMapped[2], -transformAftMapped[0], -transformAftMapped[1]);
      tf2::Quaternion geoQuatAft;
      geoQuatAft.setRPY(transformAftMapped[2], -transformAftMapped[0], -transformAftMapped[1]);
      ...
      odomAftMapped.pose.pose.orientation.x = -geoQuatA.y();
      odomAftMapped.pose.pose.orientation.y = -geoQuatA.z();
      odomAftMapped.pose.pose.orientation.z = geoQuatA.x();
      odomAftMapped.pose.pose.orientation.w = geoQuatA.w();
