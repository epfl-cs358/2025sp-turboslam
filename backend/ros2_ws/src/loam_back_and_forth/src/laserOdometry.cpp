...
#include <nav_msgs/msg/odometry.hpp> // ROS 2 message types
...
//#include <pcl/ros/conversions.h> ros 1
...
void laserCloudExtreCurHandler(const sensor_msgs::msg::PointCloud2::SharedPtr laserCloudExtreCur2)
{
  ...
  for (int i = 0; i < laserCloudExtreCur3Size; i++) {
    if (std::fabs(laserCloudExtreCur3->points[i].v - 10) < 0.005) {
    ...
    } else if (std::fabs(laserCloudExtreCur3->points[i].v - 11) < 0.005) {
    ...
    } else if (std::fabs(laserCloudExtreCur3->points[i].v - 12) < 0.005) {
    ...
    } else if (std::fabs(laserCloudExtreCur3->points[i].v - 13) < 0.005) {
...
void laserCloudLastHandler(const sensor_msgs::msg::PointCloud2::SharedPtr laserCloudLast2)
{
  ...
  if (rclcpp::Time(laserCloudLast2->header.stamp).seconds() > timeLaserCloudLast + 0.005) {
    ...
    for (int i = 0; i < laserCloudLastSize; i++) {
      if (std::fabs(laserCloudLast->points[i].v - 2) < 0.005 || std::fabs(laserCloudLast->points[i].v + 1) < 0.005) {
        laserCloudExtreLast->push_back(laserCloudLast->points[i]);
      } 
      if (std::fabs(laserCloudLast->points[i].v - 2) < 0.005 || std::fabs(laserCloudLast->points[i].v - 1) < 0.005) {
        laserCloudCornerLast->push_back(laserCloudLast->points[i]);
      } 
      if (std::fabs(laserCloudLast->points[i].v) < 0.005 || std::fabs(laserCloudLast->points[i].v + 1) < 0.005) {
        laserCloudSurfLast->push_back(laserCloudLast->points[i]);
      }
      if (std::fabs(laserCloudLast->points[i].v - 10) < 0.005) {
      ...
      if (std::fabs(laserCloudLast->points[i].v - 13) < 0.005) {
...
int main(int argc, char** argv)
{
  ...
  tf2::Stamped<Transform> laserOdometryTransStamped();
...
$
  ...
  while (status) {
    ...
    if (newLaserCloudExtreCur && newLaserCloudLast) {
      ...
      laserOdometryTrans.stamp = rclcpp::Time(timeLaserCloudLast * 1e9);
      ...
      laserOdometryTrans.stamp_ = rclcpp::Time(timeLaserCloudExtreCur * 1e9);
    ...
    if (newLaserPoints && sufficientPoints) {
      ...
      for (int iterCount = 0; iterCount < iterNum; iterCount++) {
        ...
        for (int i = 0; i < extrePointNum; i++) {
          ...
          if (std::fabs(extreOri.v + 1) < 0.05) {
            ...
            if (minPointInd2 >= 0 && minPointInd3 >= 0) {
              ...
              float s = 1;
              if (iterCount >= 30) {
                s = 1 - 8 * std::fabs(pd2) / sqrt(sqrt(extreSel.x * extreSel.x
          ...
          } else if (std::fabs(extreOri.v - 2) < 0.05) {
        ...
        if (std::fabs(matX.at<float>(0, 0)) < 0.005 &&
            std::fabs(matX.at<float>(1, 0)) < 0.005 &&
            std::fabs(matX.at<float>(2, 0)) < 0.005 &&
            std::fabs(matX.at<float>(3, 0)) < 0.01 &&
            std::fabs(matX.at<float>(4, 0)) < 0.01 &&
            std::fabs(matX.at<float>(5, 0)) < 0.01) {
    ...
    if (newLaserPoints) {
      ...
      if (sweepEnd) {
        ...
        laserCloudLast2.header.stamp = rclcpp::Time(timeLaserCloudLast);
      ...
      tf2::Quaternion geoQuaternion;
      geoQuaternion.setRPY(rz, -rx, -ry);
      geometry_msg::msg::Quaternion geoQuat;
      tf2::convert(geoQuaternion, geoQuat); //create QuaternionmsgFromRollPitchYaw doesnt exist so we need to manually create and convert
      ...
      tf2::Quaternion q;
      q.setRPY(tx,ty,tz);
      laserOdometryTrans.transform.rotation = tf2::toMsg(q);
