...
class JoystickRos2(Node):
    ...
    def __init__(self):
        ...
        self.throttle_time = 0.1 # don't send too much packets
        ...
        # Joy publisher
        self.publisher_ = self.create_publisher(Joy, 'joy', qos_profile_system_default)
        
        # individual publishers
        self.pub_joyx = self.create_publisher(Float32, 'joyx', 10)
        self.pub_joyy = self.create_publisher(Float32, 'joyy', 10)
        self.pub_accel = self.create_publisher(Float32, 'accel', 10)
        self.pub_decel = self.create_publisher(Float32, 'decel', 10)
    ...
    def publish_joy(self):
        ...
        now = time.time()
        frac, sec = modf(now)
        self.joy.header.stamp.sec     = int(sec)
        self.joy.header.stamp.nanosec = int(frac * 1e9) & 0xFFFFFFFF

        # 2) Publish full Joy
        self.publisher_.publish(self.joy)
        
        # 3) Extract axes (no clamping on triggers)
        joyx  = self.joy.axes[0]      # X‐axis (steer)
        joyy  = self.joy.axes[1]      # Y‐axis
        accel = self.joy.axes[5]      # RT trigger (forward)
        decel = self.joy.axes[2]      # LT trigger (reverse)

        # if abs(joyx) < 0.05:          # deadzone on steering only
        #     joyx = 0.0

        # 4) Lazy‐init last values & last‐sent timestamps
        ...
        # 5) Per‐topic publish guard (change or non-idle AND ≥50 ms since last)
        topics = {
            'joyx':  joyx,
            'joyy':  joyy,
            'accel': accel,
        ...
        pubs = {
            'joyx':  self.pub_joyx,
            'joyy':  self.pub_joyy,
            'accel': self.pub_accel,
            'decel': self.pub_decel,
        }

        for name, val in topics.items():
            changed_or_active = not (val == IDLE[name] and val == self.last_values[name])
            if changed_or_active and (now - self.last_sent[name] >= self.throttle_time):
                pubs[name].publish(Float32(data=val))
                self.last_values[name] = val
                self.last_sent[name] = now

        # 6) Track overall last publish time
