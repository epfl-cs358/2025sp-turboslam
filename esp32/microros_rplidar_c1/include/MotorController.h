...
{
public:
    explicit MotorController(int pwmPin) : _pin(pwmPin) {}
    bool  begin();                       // attach & arm ESC
    void  command(int8_t pct);           // –100 … +100 %
private:
    int _pin;
    Servo      _esc;
    static uint16_t percentToUs(int8_t pct) { return 1500 + int16_t(pct) * 5; }
};
